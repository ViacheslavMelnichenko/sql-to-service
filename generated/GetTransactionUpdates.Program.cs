...
// array of {"name","type","value"} objects. This proc has two datetime2 parameters,
// @LastCutoff and @NewCutoff; the runner reads them out of that array by name, so the
// differential passes the case file's params through with zero per-proc knowledge of
// argument order.
//
// Output is a JSON array of the flat result rows (customer arm then supplier arm; the
// proc has no ORDER BY). The differential passes it through corpus/canonicalise.py
// before comparing, so key order / whitespace here do not matter; values and shape do.

if (args.Length < 1)
{
    Console.Error.WriteLine("usage: GetTransactionUpdates '<paramsJson>' [mongoUri]");
    return 2;
}

var byName = ParseParams(args[0]);
var lastCutoff = RequireDateTime(byName, "LastCutoff");
var newCutoff = RequireDateTime(byName, "NewCutoff");
...
var service = new GetTransactionUpdatesService(db);
var result = service.Execute(lastCutoff, newCutoff);
...
static DateTime RequireDateTime(Dictionary<string, JsonElement> p, string name)
{
    if (!p.TryGetValue(name, out var v))
        throw new ArgumentException($"missing required parameter '{name}'");
    var raw = v.ValueKind == JsonValueKind.String ? v.GetString()! : v.GetRawText();
    // Parse the wall-clock datetime with no timezone assumption (RoundtripKind keeps
    // the literal value); the service compares it against the seed's UTC dates.
