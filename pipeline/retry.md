...
- Where the cap is enforced and the outcome recorded: `evals/harness.py` (built and
  running — it counts attempts across the loop and, after the cap, records the proc
  with its last gate output; the hooks deliver the in-loop feedback within each
  attempt).
